...
class SavaMaster():
    ...
    def initialize(self, source_node, members):
        self.set_workers(members)
        self.init_work(source_node)
    ...
    def init_work(self, source_node):
        print('master start initializing')
        print(self._workers)
        for worker_id, worker_ip in self._workers.items():
            handle = get_tcp_client_handle(worker_ip)
            handle.init_sava_worker(worker_id, source_node, self._workers)
...
class SavaWorker():
    ...
    def initialize(self, worker_id, source_node, workers):
    ...
    def load_graph(self, init_value):
        ...
        with open(SAVA_GRAPH_FILE) as fin:
            for cnt, line in enumerate(fin):
                ...
                if (self.node_belong_to(v1) == self.worker_id):
                    self._graph[v1].append(v2)
                    self._nodeValue[v1] = init_value
                if (self.node_belong_to(v2) == self.worker_id):
                    self._graph[v2].append(v1)
                    self._nodeValue[v2] = init_value
    ...
    def store_to_msgin(self, msg):
        ...
        for node in msg.keys():
            self._msgin[node].append(min(msg[node]))
        self._lock.release()
